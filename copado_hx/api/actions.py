...
class ActionsApiClient:
    ...
    def commit(self, story_id: str, message: str) -> dict:
        return self._run_job_template(self.ACTIONS["commit"], story_id=story_id, message=message)

    def promote(self, story_id: str, environment: str, validate_only: bool = False) -> dict:
        return self._run_job_template(
            self.ACTIONS["promote"], validate_only=validate_only,
            story_id=story_id, environment=environment,
        )

    def deploy(self, story_id: str, environment: str) -> dict:
        return self._run_job_template(self.ACTIONS["deploy"], story_id=story_id, environment=environment)
